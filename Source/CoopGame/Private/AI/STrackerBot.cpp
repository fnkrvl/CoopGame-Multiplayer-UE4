...
FAutoConsoleVariableRef CVarCVARDebugTrackerBotDrawing(
	TEXT("COOP.DebugTrackerBot"),
	DebugTrackerBotDrawing,
	TEXT("Draw Debug Lines for TrackerBotWeapons"),
	ECVF_Cheat
);
...
void ASTrackerBot::BeginPlay()
{
	...
	if (HasAuthority())
	{
		...
		// Every seond we update our power-level based on nearby bots (CHALLENGE CODE)
...
void ASTrackerBot::OnCheckNearbyBots()
{
	...
	PowerLevel = FMath::Clamp(NroBots, 0, MaxPowerLevel);
	...
	if (MatInst)
	{
		// Convert to a float between 0 and 1 just like an 'Alpha' value of a texture. Now the material can bee set up without having to know the max power lenth
		...
		// Note: static_cast<float>(MaxPowerLevel) converts the int32 to a float,
